...
# ฟังก์ชันนับจำนวนคำภาษาไทยอย่างแม่นยำ
# คัดกรองเครื่องหมาย Markdown, ช่องว่าง และโค้ดออก แล้วตัดคำตามหลักพจนานุกรม
count_thai_words() {
    local file="$1"
    ...
    # 1. ล้างเครื่องหมาย Markdown และข้อความที่ไม่ใช่เนื้อหาหลักออกก่อน
    local cleaned_text=$(cat "$file" | \
        sed '/^```/,/^```/d' | \
        sed 's/^#\+[[:space:]]*//' | \
        sed 's/\*\*//g' | \
        sed 's/__//g' | \
    ...
    # 2. ใช้ Python ร่วมกับ Regex ในการตัดคำไทย (หรือเปลี่ยนเป็น pythainlp ได้หากติดตั้งไว้)
    # วิธีนี้จะดึงกลุ่มคำภาษาไทยและคำภาษาอังกฤษแยกออกจากกันแล้วนับรวมเป็นคำ
    local word_count=$(python3 -c "
import re, sys
text = \"\"\"$cleaned_text\"\"\"
# คัดเลือกเฉพาะกลุ่มคำภาษาไทย คำภาษาอังกฤษ และตัวเลข
words = re.findall(r'[ก-์]+|[a-zA-Z0-9]+', text)
print(len(words))
" 2>/dev/null || echo "0")
...
# แสดงข้อมูลสถิติจำนวนคำภาษาไทย
show_word_count_info() {
    local file="$1"
    local min_words="${2:-0}"
    local max_words="${3:-999999}"
    local actual_words=$(count_thai_words "$file")

    echo "จำนวนคำ：$actual_words"

    if [ "$min_words" -gt 0 ]; then
        if [ "$actual_words" -lt "$min_words" ]; then
            echo "⚠️ จำนวนคำยัง不ถึงเกณฑ์ขั้นต่ำที่กำหนด（ขั้นต่ำ: ${min_words} คำ）"
